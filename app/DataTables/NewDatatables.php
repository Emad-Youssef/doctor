...
class NewDatatables extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addColumn('checkbox', 'dashboard.news.btn.check')
            ->addColumn('status', 'dashboard.news.btn.status')
            ->addColumn('img', 'dashboard.news.btn.img')
            ->addColumn('slider', 'dashboard.news.btn.slider')
            ->addColumn('action', 'dashboard.news.btn.button')
            ->rawColumns(['checkbox','status','img', 'slider', 'action']);
    }
    ...
    {
        return $this->builder()
                    ->setTableId('newdatatables-table')
    ...
    protected function getColumns()
    {
        return [
            ...
            ->addClass('text-center'),
            Column::computed('slider')
            ->exportable(false)
            ->printable(false)
            ->width(150)
            ->title(trans('site.on_slider'))
            ->addClass('text-center'),
    ...
    {
        return 'NewDatatables_' . date('YmdHis');
    }
}
