...
@section('content')
    ...
    <section class="content">
        <div class="box">
                ...
                <form class="form-ajax" data-route="{{ route('dashboard.rosheta.update',$rosheta->id ) }}" method="post"  data-token="{{ csrf_token() }}" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="form-group col col-md-6">
                            ...
                            <select class="form-control select2" name="patient_id">
                                ...
                                @foreach($patients as $patient)
                                    <option value="{{$patient->id}}" {{$rosheta->patient_id == $patient->id?'selected':'' }}>{{ $patient->pocket->f_name }} {{ $patient->pocket->l_name }}--> {{ $patient->pocket->y }} / {{ $patient->pocket->m }} / {{ $patient->pocket->d }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group col col-md-6">
                        ...
                        <textarea name="diagnosis"  class="form-control summernote">{{$rosheta->diagnosis}}</textarea>
                        </div>
                        <div class="form-group col col-md-6">
                        <label>@lang('site.medication_past')</label>
                        <div class="card">
                            @foreach($roshDrug as $dr)
                                <input type="text" class="form-control col col-md-6 text-left" disabled value="{{$dr->drug_name}}" class="card-text">
                                <input type="text" class="form-control col col-md-6 text-left" disabled value="{{$dr->drug_use}}" class="card-text"><br>
                            @endforeach
                        </div>
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col col-md-6">
                            ...
                            <input id="doctor_name_input_inside" type="text" name="doctor_name" class="form-control" value="{{$rosheta->doctor_name}}" placeholder="@lang('site.doctor')">
                        </div>
                    </div>
